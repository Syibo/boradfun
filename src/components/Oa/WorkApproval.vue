  ...
  <el-dialog :visible.sync="visible" width="60%" :close-on-click-modal="false" :show-close="false" class="contract-from" @close="closeVisble">
    <span slot="title" class="dialog-title">
      ...
      <div class="dialog-title-right">
        ...
        <el-button v-if="title === '申请加班' && active !== 3" @click="submitForm(0)">拒 绝</el-button>
        <el-button v-if="active !== workflow.length" size="small" type="primary" @click="submitForm(1)">{{ title === '申请加班' ? '同 意' : '确 定' }}</el-button>
      ...
      <div v-else>
        <el-form ref="ruleForm" label-position="top" :model="ruleForm" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-row>
            <el-row :gutter="20">
              <el-col :span="24">
                <el-form-item label="实际加班时长" prop="real">
                  <el-input
...
export default {
  ...
  methods: {
    ...
    async timeCheckFun(status) {
      const res = await putOneOverTimeCheck({ id: this.id, real: this.ruleForm.real })
      if (res.ret === 0) {
